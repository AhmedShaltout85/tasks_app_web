...
//                     task.visitPlace,
//                     const Color.fromARGB(255, 12, 80, 40),
//                   ),
//                   const SizedBox(height: 8),
...
class _ReportScreenState extends State<ReportScreen>
  ...
  String? selectedDepartment;
  String? selectedComplaintType;
  String? selectedCoOperator; // New filter for co-operator
  bool _isFilterExpanded = true;
  late AnimationController _animationController;
  late Animation<double> _fadeAnimation;
  ...
  void initState() {
    ...
    selectedDepartment = 'الكل';
    selectedComplaintType = 'الكل';
    selectedCoOperator = 'الكل'; // Initialize co-operator filter
  ...
  /// Returns the co-operator list - shows all users except the current user
  List<String> _buildCoOperatorList({
    required String userUsername,
    required List<Map<String, String>> allUsers, // [{username, department}]
  }) {
    // Helper: strip blocked accounts
    ...
    // Get all usernames except the current user and blocked accounts
    final allUsernames = allUsers
        .map((u) => u['username']!)
        .where((username) =>
                isAllowed(username) &&
                username != userUsername // Exclude current user
            )
        .toSet()
        .toList();

    // Sort the list alphabetically for better UX
    allUsernames.sort((a, b) => a.compareTo(b));
  ...
  List<DailyTaskModel> getFilteredData(
    ...
    return tasks.where((task) {
      ...
      // ── Co-operator filter ─────────────────────────────────────────────────
      if (selectedCoOperator != null && selectedCoOperator != 'الكل') {
        // Check if the selected co-operator is in the task's coOperator list
        if (!task.coOperator.contains(selectedCoOperator!)) return false;
      }
  ...
  void _clearFilters({
    ...
    setState(() {
      ...
      selectedIsRemote = 'الكل';
      selectedComplaintType = 'الكل';
      selectedCoOperator = 'الكل'; // Reset co-operator filter
      ...
      body: ResponsiveContentContainer(
        child: Consumer<DailyTaskProvider>(
          builder: (context, taskProvider, child) {
            ...
            // Build co-operator list - shows all users except current user
            final coOperatorList = _buildCoOperatorList(
              userUsername: userUsername,
              allUsers: allUsers,
            );
            ...
            // Guard: if current selectedCoOperator is no longer in list, reset
            if (!coOperatorList.contains(selectedCoOperator)) {
              selectedCoOperator = coOperatorList.first;
            }
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        Expanded(
                                          child: _buildDropdown(
                                            label: 'زميل متعاون',
                                            value: coOperatorList.contains(
                                                    selectedCoOperator)
                                                ? selectedCoOperator!
                                                : coOperatorList.first,
                                            items: coOperatorList,
