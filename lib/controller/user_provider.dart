...
class UserProvider with ChangeNotifier, WidgetsBindingObserver {
  ...
  void _setupCallbacks() {
    ...
    // Cross-tab broadcast wiring: any successful token save (sign-in,
    // resume-refresh, reactive/proactive refresh -- in any tab, leader
    // or not) tells other tabs, and an explicit sign-out tells them too.
    //
    // This is deliberately wired HERE rather than inside DioClient's own
    // constructor. By the time this method's body runs, both `_api` and
    // `_auth` (declared as `final` fields above) are already fully
    // constructed -- Dart initializes instance fields before running the
    // constructor body -- so neither AuthStateManager nor DioClient is
    // still mid-construction. Wiring it inside DioClient's constructor
    // instead caused a circular-singleton-construction crash
    // (LateInitializationError on DioClient.instance), because it forced
    // AuthStateManager to build while DioClient's own `instance` field
    // assignment was still in progress.
    _auth.onTokensSaved = (access, refresh) {
      dioClient.setToken(access);
      dioClient.setRefreshToken(refresh);
      dioClient.broadcastTokens(access, refresh);
    };
    _auth.onSessionCleared = () {
      dioClient.broadcastExpired();
    };
  }
  ...
  Future<void> clearUserData() async {
    ...
    _users = [];
    _error = null;
    // NOTE: _auth.clearSession() now broadcasts 'EXPIRED' to other tabs
    // by default (see FIX #4 in AuthStateManager/DioClient), so an
    // explicit sign-out here correctly signs the user out everywhere,
    // not just in this tab. No change needed at this call site.
  ...
  Future<void> signIn({
    ...
    try {
      ...
      // FIX #5: pass the raw nullable refreshToken through instead of
      // `refreshToken ?? ''`. AuthStateManager.setTokensFromSignIn now
      // accepts String? and correctly treats "no refresh token" as
      // "not authenticated for refresh purposes" rather than silently
      // storing an empty string that later passes every null-check
      // guard and triggers a doomed refresh call.
